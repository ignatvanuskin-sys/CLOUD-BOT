...
describe('api hardening smoke', () => {
  ...
  it('auth dev-login, order snapshot, payment idempotency and protected download', async () => {
    ...
    await request(app).post('/api/webhooks/telegram').set('x-telegram-bot-api-secret-token','test-secret-token-123').send({ update_id: 3, message: { from: { id: 7 }, successful_payment: { invoice_payload: row.payload, currency: 'XTR', total_amount: 100, telegram_payment_charge_id: 'ch1' } } }).expect(200);
    await request(app).post('/api/webhooks/telegram').set('x-telegram-bot-api-secret-token','test-secret-token-123').send({ update_id: 3, message: { from: { id: 7 }, successful_payment: { invoice_payload: row.payload, currency: 'XTR', total_amount: 100, telegram_payment_charge_id: 'ch1' } } }).expect(200);
