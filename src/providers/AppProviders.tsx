...
function SessionProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    setAuthRefreshHandler(initData && isTelegram() ? async () => { const auth = await authTelegram(initData); session.set(); queryClient.setQueryData(['me'], { user: auth.user }); return 'cookie'; } : null);
    return () => setAuthRefreshHandler(null);
  }, []);
  useEffect(() => { let active = true; initTelegram(); async function boot() { try { const initData = telegram?.initData; if (!session.get() && initData && isTelegram()) { await authTelegram(initData); session.set(); } if (!session.get()) setGuest(true); } catch (error) { console.error('auth_bootstrap_failed', error); setGuest(true); } finally { if (active) setBooting(false); } } void boot(); return () => { active = false; }; }, []);
  const me = useQuery({ queryKey: ['me'], queryFn: async () => { try { return await getMe(); } catch (error) { const initData = telegram?.initData; if (!session.get() && initData && isTelegram()) { await authTelegram(initData); session.set(); return getMe(); } throw error; } }, enabled: !booting && Boolean(session.get()), retry: false });
