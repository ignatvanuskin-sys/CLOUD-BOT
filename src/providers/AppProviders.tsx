...
function SessionProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { let active = true; initTelegram(); async function boot() { try { const initData = telegram?.initData; if (!session.get() && initData && isTelegram()) { const auth = await authTelegram(initData); if (auth.token) session.set(auth.token); } if (!session.get()) setGuest(true); } catch (error) { console.error('auth_bootstrap_failed', error); setGuest(true); } finally { if (active) setBooting(false); } } void boot(); return () => { active = false; }; }, []);
