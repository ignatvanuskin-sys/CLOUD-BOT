...
import { Award, Bell, BellRing, ChevronRight, Copy, Crown, Fingerprint, FolderPlus, Globe2, Heart, History, LockKeyhole, LogOut, ReceiptText, Settings, Shield, Trash2, Users, MoreHorizontal, Sparkles, CheckCircle2 } from 'lucide-react';
...
export function ProfilePage() {
  ...
  return <div className="page profile-page">
    {access.data?.canCreateProjects ? <Button className="primary profile-sell-cta" onClick={() => nav('/admin/projects')}><FolderPlus />Выставить товар</Button> : null}
...
export function SettingsPage() {
  ...
  async function erase() { if (await confirmPopup('Очистить данные', 'Избранное и локальные настройки будут удалены с этого устройства.')) { localStorage.clear(); location.reload(); } }
  ...
  return <div className="page settings-page">
    <section className="settings-hero"><div className="settings-hero-icon"><Settings /></div><div><span className="eyebrow">Telegram Mini App</span><h1>Настройки</h1><p>Только необходимые параметры. Внешний вид следует теме Telegram.</p></div></section>
    ...
    <SettingsGroup title="Основные" caption="Параметры, которые влияют на ежедневное использование"><Setting icon={Bell} title="Уведомления" text="Обновления покупок и новые версии"><Switch label="Уведомления" checked={preferences.notifications} onChange={v => update({ notifications: v })} /></Setting><Setting icon={Shield} title="Приватный режим" text="Скрывать чувствительные детали профиля"><Switch label="Приватный режим" checked={preferences.privateMode} onChange={v => update({ privateMode: v })} /></Setting></SettingsGroup>
    <SettingsGroup title="Безопасность" caption="Дополнительная защита доступа к покупкам"><Setting icon={Fingerprint} title="Биометрия" text={bio ? 'Подтверждена на этом устройстве' : 'Если поддерживается Telegram'}><Button className="compact" onClick={biometry}>{bio ? 'Активна' : 'Настроить'}</Button></Setting></SettingsGroup>
...
function Setting({ icon: Icon, title, text, children }: { icon: React.ComponentType<{ size?: number }>; title: string; text: string; children: React.ReactNode }) { return <div className="setting-row"><span className="setting-icon"><Icon /></span><div><b>{title}</b><small>{text}</small></div><div className="setting-control">{children}</div></div>; }
