...
export default function ProductPage(){
 ...
 return <div className="page product-page"><section className="product-hero"><div className="product-cover">{product.preview&&<img src={product.preview} alt="" loading="lazy" decoding="async"/>}<motion.div animate={{rotate:[-3,3,-3],y:[0,-6,0]}} transition={{duration:6,repeat:Infinity,ease:'easeInOut'}}><PackageCheck/></motion.div><Badge>{product.type.replace('_',' ')}</Badge></div><button className={liked?'favorite-fab liked':'favorite-fab'} aria-label="Избранное" onClick={toggleFavorite}><Heart fill={liked?'currentColor':'none'}/></button><span className="eyebrow">{product.category} · v{product.version}</span><h1>{product.title}</h1><p className="lead">{product.result}</p>{product.demo_url&&<Button className="soft" onClick={()=>telegram?.openLink?.(product.demo_url)}><ExternalLink/>Открыть демо</Button>}</section>
