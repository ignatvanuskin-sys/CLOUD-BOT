...
for (const [id, title, result, type, cat] of items) {
  ...
  for (const plan of [['starter',299,1,0,14,30],['pro',799,1,1,30,90],['agency',1999,5,1,60,180]]) await db.prepare('insert or replace into license_plans(id,product_id,name,price_xtr,projects,commercial,support_days,updates_days,terms) values(?,?,?,?,?,?,?,?,?)').run(`${id}-${plan[0]}`,id,String(plan[0]).toUpperCase(),plan[1],plan[2],plan[3],plan[4],plan[5],'Нельзя перепродавать исходник отдельно. Можно изменять код под разрешённые проекты.');
  const asset = await ensureDemoAsset(id, '1.0.0');
  await db.prepare('insert or replace into product_assets(id,product_id,version,storage_key,file_name,mime_type,size_bytes,checksum_sha256,status) values(?,?,?,?,?,?,?,?,?)').run(`${id}-asset-1`, id, '1.0.0', asset.key, 'demo-package.txt', 'text/plain', asset.size, asset.checksum, 'published');
}
for (const id of (process.env.ADMIN_TELEGRAM_IDS || '').split(',').filter(Boolean)) await db.prepare('insert or ignore into admin_users(telegram_id,role) values(?,?)').run(id.trim(), 'owner');
