...
class S3StorageAdapter implements StorageAdapter {
  ...
  async createDownloadUrl(key: string, ttlSeconds: number, fileName: string) { const c = loadConfig(); const safeFileName = fileName.replace(/[\r\n"]/g, '').slice(0, 180) || 'download.bin'; return getSignedUrl(this.s3, new GetObjectCommand({ Bucket: c.S3_BUCKET, Key: key, ResponseContentDisposition: `attachment; filename="${safeFileName}"` }), { expiresIn: ttlSeconds }); }
