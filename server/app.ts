...
export function createApp() {
  ...
  app.get('/api/products/:slug', async (req, res) => { const p = await db.prepare("select * from products where (slug=? or id=?) and status='published'").get(req.params.slug, req.params.slug) as any; if (!p) return safeError(res, 404, 'not_found', 'РўРѕРІР°СЂ РЅРµ РЅР°Р№РґРµРЅ'); res.json({ product: p, plans: await db.prepare('select * from license_plans where product_id=? order by price_xtr').all(p.id) }); });
  ...
  app.post('/api/orders', user, limiter, async (req: any, res) => {
    const plan = await db.prepare('select * from license_plans where id=?').get(req.body.licenseId) as any; if (!plan) return safeError(res, 404, 'license_not_found', 'Р›РёС†РµРЅР·РёСЏ РЅРµРґРѕСЃС‚СѓРїРЅР°');
    const product = await db.prepare("select * from products where id=? and status='published'").get(plan.product_id) as any; if (!product) return safeError(res, 404, 'product_not_found', 'РўРѕРІР°СЂ РЅРµРґРѕСЃС‚СѓРїРµРЅ');
  ...
  app.post('/api/orders/:id/invoice', user, limiter, async (req: any, res) => {
    const order = await db.prepare('select * from orders where id=? and user_id=?').get(req.params.id, req.userId) as any; if (!order) return safeError(res, 404, 'order_not_found', 'Р—Р°РєР°Р· РЅРµ РЅР°Р№РґРµРЅ'); if (order.status !== 'pending') return safeError(res, 409, 'order_not_pending', 'Р—Р°РєР°Р· СѓР¶Рµ РѕР±СЂР°Р±РѕС‚Р°РЅ');
    let link = `tg://invoice?slug=test-${order.id}`; if (bot) link = await bot.api.createInvoiceLink(`РџРѕРєСѓРїРєР°: ${order.product_title}`, 'Р¦РёС„СЂРѕРІРѕР№ С‚РѕРІР°СЂ. Р”РѕСЃС‚СѓРї РІС‹РґР°С‘С‚СЃСЏ РїРѕСЃР»Рµ successful_payment.', order.payload, '', 'XTR', [{ label: order.product_title, amount: order.amount_xtr }]);
  ...
  app.post('/api/webhooks/telegram', async (req: any, res) => {
    ...
    if (update.pre_checkout_query) {
      ...
      if (!ok) log('warn','pre_checkout_rejected',{ payload: q.invoice_payload?.slice(0,20) });
    ...
    if (payment) {
      if (update.update_id != null) {
        const inserted = await db.prepare('insert or ignore into webhook_updates(update_id) values(?)').run(String(update.update_id));
        if (inserted.changes === 0) return res.json({ ok: true, duplicate: true });
      }
      const order = await db.prepare('select * from orders where payload=?').get(payment.invoice_payload) as any;
      const valid = order && payment.currency === 'XTR' && Number(payment.total_amount) === Number(order.amount_xtr);
      if (valid && order.status !== 'paid' && order.status !== 'fulfilled') {
        await db.transaction(async () => {
          await db.prepare("update orders set status='fulfilled',telegram_charge_id=?,paid_at=CURRENT_TIMESTAMP,fulfilled_at=CURRENT_TIMESTAMP where id=? and status='pending'").run(payment.telegram_payment_charge_id, order.id);
          await db.prepare('insert or ignore into entitlements(id,user_id,product_id,license_id,order_id) values(?,?,?,?,?)').run(nanoid(), order.user_id, order.product_id, order.license_id, order.id);
        });
        await event(order.user_id, 'payment_success', order.product_id, { orderId: order.id });
      } else if (!valid) {
        log('warn', 'payment_rejected', { payload: payment.invoice_payload?.slice(0,20) });
      }
  ...
  app.post('/api/purchases/:id/download', user, limiter, async (req: any, res) => {
    const e = await db.prepare('select e.*,p.version from entitlements e join products p on p.id=e.product_id where e.id=? and e.user_id=? and e.active=1').get(req.params.id, req.userId) as any; if (!e) return safeError(res, 404, 'entitlement_not_found', 'РџРѕРєСѓРїРєР° РЅРµ РЅР°Р№РґРµРЅР°');
  ...
  app.post('/api/admin/assets/upload', user, adminRole(['owner','editor']), limiter, upload.single('file'), async (req: any, res) => {
    ...
    if (!file || !productId || !version) return safeError(res, 400, 'upload_required', 'РќСѓР¶РµРЅ С„Р°Р№Р», productId Рё version');
    const product = await db.prepare('select * from products where id=?').get(productId) as any; if (!product) return safeError(res, 404, 'product_not_found', 'РўРѕРІР°СЂ РЅРµ РЅР°Р№РґРµРЅ');
  ...
  app.post('/api/admin/assets/:id/publish', user, adminRole(['owner','editor']), limiter, async (req: any, res) => { const asset = await db.prepare('select * from product_assets where id=?').get(req.params.id) as any; if (!asset || !['approved','published'].includes(asset.status)) return safeError(res, 409, 'asset_not_approved', 'Asset РЅРµ РїСЂРѕС€С‘Р» РїСЂРѕРІРµСЂРєСѓ'); await db.prepare("update product_assets set status='published' where id=?").run(asset.id); await audit(req.userId,'asset_publish','asset',asset.id); res.json({ ok: true }); });
