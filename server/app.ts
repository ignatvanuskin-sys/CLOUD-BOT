...
export function createApp() {
  ...
  app.get('/api/products', catalogLimiter, async (req, res) => {
    ...
    let sql = "select p.id,p.slug,p.type,p.category,p.title,p.result,p.description,p.stack,p.demo_url,p.version,p.changelog,l.price_from from products p left join (select product_id,min(price_xtr) price_from from license_plans group by product_id) l on l.product_id=p.id where p.status='published' and p.type<>'template'"; const args: any[] = [];
  ...
  app.get('/api/products/:slug', catalogLimiter, async (req, res) => { const p = await db.prepare("select * from products where (slug=? or id=?) and status='published' and type<>'template'").get(req.params.slug, req.params.slug) as any; if (!p) return safeError(res, 404, 'not_found', 'Товар не найден'); res.json({ product: p, plans: await db.prepare('select * from license_plans where product_id=? order by price_xtr').all(p.id) }); });
